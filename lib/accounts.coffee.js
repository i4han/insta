...
module.exports.accounts = {
  login: {
    styl$: function(C, _) {
      return _.slice("#login-buttons|>float right|border 0 ~\n|<#login-buttons + li .dropdown-menu |>float right|right 0|left auto");
    },
    jade: function(C, _) {
      return _.slice("ul.nav.navbar-nav#login-buttons|>+Template.dynamic(template=template)");
    },
    events: {
  ...
  dropdown_menu_logged_in: {
    ...
    jade: function(C, _) {
      return _.slice("each items|>+menu");
    },
    helpers: {
      items: function() {
        return [
  ...
  login_messages: {
    styl$: function(C, _) {
      return _.slice("#login-dropdown .alert|>padding 6px|margin-bottom 14px");
    },
